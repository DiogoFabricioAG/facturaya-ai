...
Los datos de empresas y las claves SOL ya no están en `.env`.
...
# Edita .env y define como mínimo APP_URL, APP_KEY, DB_PASSWORD,
# PLATFORM_ADMIN_TOKEN y, si corresponde, OPENAI_API_KEY.
...
- genera `APP_KEY` una sola vez y consérvala en las copias de seguridad;
- usa contraseñas aleatorias para PostgreSQL y `PLATFORM_ADMIN_TOKEN`;
- deja `APP_DEBUG=false`;
- rota cualquier clave que haya sido usada durante desarrollo;
- realiza copias de los volúmenes `postgres_data` y `app_storage`;
